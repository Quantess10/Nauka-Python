...
def validate_length(action, value_if_allowed, prior_value, text, validation_type, trigger_type, widget_name):
    ...
    if action == '1':  # Tylko przy wstawianiu tekstu
        # Sprawdź, czy po wstawieniu długość tekstu nie przekroczy 12 znaków
        total_length = len(prior_value) + len(text)
        if total_length > 12:
            messagebox.showwarning("Ostrzeżenie", "Kod kreskowy może zawierać maksymalnie 12 cyfr.")
...
def generate_barcode():
    ...
    if len(text) != 12:
        messagebox.showwarning("Ostrzeżenie", "Kod kreskowy musi składać się z 12 cyfr.")
        return
    if text == '' or text == 'Wpisz tekst tutaj...':
        messagebox.showinfo("Error! Error! Dziura w samolocie!", "Nie wygeneruję Ci kodu z niczego! Wpisz coś :)")
        return
    ...
    try:
        ...
        messagebox.showerror("Błąd", "Nieprawidłowe dane. Wpisz 12 cyfrowy kod!")
        return
